...
int main(int argc, char* argv[]) {
	//Once the program is running, a chess game using the default chess board is created
	Chess::Game game;
	// Display command options
	show_commands();

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	/*Prior to the user selecting an action, present the current state of the board to the user on standard output*/

	//Keep playing until the game is over (when the game reaches checkmate or stalemate, or the user elects to quit)
	...
	while(!game_over) {
		...
		// If the board is in a stalemate state, notify the players, end the game
		...
		if (choice.length() != 1) {
			...
			case 'L': case 'l': {
				...
				// Handle any file i/o and gmae loading problem, exit the program with return code -1 if an exception is caught
				std::string argument;
				try{
					std::cin >> argument;
					std::ifstream ifs;
			...
			case 'M': case 'm': {
				...
				// And make the move
          		...
          		// Add try catch blocks to recover from illegal moves
		  			try{
